...
class Process {
    ...
    async waitForLineOutput(regex, timeout = 0) {
        ...
        let pErr = new Promise((resolve, reject) => {
            ...
            function onClose(error) {
                cleanup();
                if ([exports.CDP_WEBSOCKET_ENDPOINT_REGEX, exports.WEBDRIVER_BIDI_WEBSOCKET_ENDPOINT_REGEX].includes(regex)) {
        ...
        const rl2 = readline_1.default.createInterface(this.#browserProcess.stdout || this.#browserProcess.stderr);
        let stdout = '';
        let pOut = new Promise((resolve, reject) => {
            rl2.on('line', onLine);
            rl2.on('close', onClose);
            this.#browserProcess.on('exit', onClose);
            this.#browserProcess.on('error', onClose);
            const timeoutId = timeout > 0 ? setTimeout(onTimeout, (timeout - 15000)) : undefined;
            const cleanup = () => {
                if (timeoutId) {
                    clearTimeout(timeoutId);
                }
        ...
        try {
            ...
            while (true) {
                if (isResolvedErr || isResolvedOut)
                    return finalResult;
                await sleep(1000);
            }
        }
